...
class DeepFashionDataset(Dataset):
    ...
    Estructura esperada en root_dir:
        ...
        ├── Anno/
        │   ├── list_category_img.txt     # imagen → índice de categoría
        │   └── list_category_cloth.txt   # índice → nombre de categoría
        └── Eval/
            └── list_eval_partition.txt   # train / val / test split
    """

    def __init__(self, root_dir: str, partition: str = 'train', transform=None):
        """
        Args:
        ...
        cat_path = os.path.join(root_dir, 'Anno', 'list_category_img.txt')
        cat_df = pd.read_csv(
            cat_path, skiprows=1, sep=r'\s+',
        ...
        cloth_path = os.path.join(root_dir, 'Anno', 'list_category_cloth.txt')
        cloth_df = pd.read_csv(
            cloth_path, skiprows=1, sep=r'\s+',
            names=['category_name', 'category_type']
        )
        self.category_names = cloth_df['category_name'].tolist()

        print(f"[{partition.upper()}] {len(self.data):,} imágenes | "
...
if __name__ == '__main__':
    ...
    DATA_ROOT = '../data/raw'   # Ajusta esta ruta
